...
@log
def is_client_here(account_name='Guest'):
...
def main():
    ...
    try:
        ...
        message_out(transport, is_client_here())
        ...
        while True:
            ...
            if client_mode == 'send':
                try:
                    message_out(transport, create_message(transport))
            ...
            if client_mode == 'listen':
                try:
                    message_from_server(message_in(transport))
                except (ConnectionResetError, ConnectionError, ConnectionAbortedError):
                    LOGGER.error(f'Соединение с сервером {server_address} было потеряно.')
                    sys.exit(1)
